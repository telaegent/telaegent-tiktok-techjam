 ...
 */
const makePorts = (temporaryRoot: string): TelaegentPorts => ({
...
beforeEach(async () => {
  workspaceRoot = await mkdtemp(path.join(tmpdir(), "telaegent-ws-"));
  audit = [];
...
const initialize = async (agentId: string, branch: string) => {
  ...
  // The Starter Kit's WorkspaceManager writes these before Telaegent runs.
