...
describe("Telagent envelope schema", () => {
  it("accepts a valid operation-specific envelope", () => {
    expect(telagentEnvelopeSchema.parse(validEnvelope()).operation).toBe(
  ...
  it("rejects unknown versions and operation/payload mismatches", () => {
    expect(
      telagentEnvelopeSchema.safeParse({ ...validEnvelope(), schemaVersion: "telagent.v2" })
        .success,
    ).toBe(false);
    const mismatch = validEnvelope();
    mismatch.operation = "relay_update_progress";
    expect(telagentEnvelopeSchema.safeParse(mismatch).success).toBe(false);
  });

  it("rejects permission injection at envelope and payload boundaries", () => {
    expect(
      telagentEnvelopeSchema.safeParse({
        ...validEnvelope(),
  ...
  it("requires a distinct recipient for cross-Agent operations", () => {
    ...
    expect(telagentEnvelopeSchema.safeParse(request).success).toBe(false);
    expect(
      telagentEnvelopeSchema.safeParse({
        ...request,
  ...
  it("requires matching reply identifiers", () => {
    ...
    expect(telagentEnvelopeSchema.safeParse(reply).success).toBe(false);
    reply.delivery.replyToRequestId = "req_status_01";
    expect(telagentEnvelopeSchema.safeParse(reply).success).toBe(true);
  });
...
describe("purpose-specific output and HTTP decision schemas", () => {
  ...
  it("defines a strict body schema for every mutating Telagent route", () => {
    expect(Object.keys(telagentHttpBodySchemas)).toEqual([
      "conversationMessage",
      "continueIntent",
      "completeIntent",
