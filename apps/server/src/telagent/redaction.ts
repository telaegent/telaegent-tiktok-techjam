...
function isTypeAnnotation(separator: string, value: string): boolean {
  // TypeScript field declarations use `:` and never quote the type. Preserve
  // those two syntax boundaries so assigned values such as `apiKey = string`
  // or `apiKey: "string"` cannot use the type allowlist to evade redaction.
  if (!separator.includes(":") || separator.includes("=") || /^["']/.test(value)) {
    return false;
  }
