...
function executionFingerprintInput(request: TelagentRequest): unknown {
  return {
...
export function fingerprintRequest(request: TelagentRequest): string {
  return createHash("sha256")
    .update(canonicalize(executionFingerprintInput(request)))
    .digest("hex");
}

export function idempotencyScope(request: TelagentRequest): string {
  return [
    request.projectId,
  ...
  return {
    ...
    expiresAt: new Date(
      Math.max(Date.parse(request.delivery.expiresAt), Date.parse(createdAt)) +
        TELAGENT_LIMITS.idempotencyRetentionMs,
    ).toISOString(),
    createdAt,
  };
}

export function assertRequestTiming(request: TelagentRequest, now: string): void {
  const createdAt = Date.parse(request.delivery.createdAt);
  const expiresAt = Date.parse(request.delivery.expiresAt);
  const currentTime = Date.parse(now);
  if (createdAt > currentTime + TELAGENT_LIMITS.maxClockSkewMs) {
