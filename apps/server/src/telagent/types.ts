...
export type TelagentToolName = (typeof TELAGENT_TOOL_NAMES)[number];
...
export type TelagentErrorCode = (typeof TELAGENT_ERROR_CODES)[number];
...
export interface TelagentEnvelopeBase<
  TOperation extends TelagentToolName,
  TPayload,
> {
...
export type TelagentEnvelope<
  TPayload,
  TOperation extends TelagentToolName = TelagentToolName,
> = TelagentEnvelopeBase<TOperation, TPayload>;

/** Public request union. `operation` always determines the exact payload schema. */
export type TelagentRequest = {
  [TOperation in TelagentToolName]: TelagentEnvelopeBase<
    TOperation,
    PayloadByOperation[TOperation]
  >;
}[TelagentToolName];

export type TelagentToolAction = {
  [TOperation in TelagentToolName]: {
    name: TOperation;
    arguments: PayloadByOperation[TOperation];
  };
}[TelagentToolName];

export interface AgentStep<TAction extends TelagentToolAction = TelagentToolAction> {
  publicSummary: string;
  nextAction: TAction | null;
  taskState: AgentTaskState;
}

type ActionNamed<TName extends TelagentToolName> = Extract<
  TelagentToolAction,
  { name: TName }
>;
...
export interface ConversationEntryPayloadByType {
  ...
  tool_call: { action: TelagentToolAction };
  tool_result: {
    toolName: TelagentToolName;
  ...
  error: { code: TelagentErrorCode; message: string; auditEventId?: string | undefined };
}
...
export interface Operation {
  operationId: string;
  type: TelagentToolName | "agent_run" | "workflow";
  projectId: string;
  conversationId: string;
  requestId: string;
  correlationId: string;
...
export interface TelagentDatabase {
  projects: Project[];
  owners: Owner[];
...
export interface TelagentErrorBody {
  code: TelagentErrorCode;
  message: string;
  safeDetails?: SafeJsonObject | undefined;
  correlationId: string;
...
export interface TelagentErrorEnvelope {
  error: TelagentErrorBody;
}
