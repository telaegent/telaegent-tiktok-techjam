...
export async function initializePhoenixWorkspace(
  ...
  if (!(await ports.fs.exists(path.join(fixtureRoot, ".telagent", "project.json")))) {
  ...
  const base = await createCheckpointCommit(
    workspacePath,
    "Phoenix Web App baseline (Telagent demo fixture)",
    ports.git,
  );
...
export async function resetPhoenixWorkspace(
  ...
  const marker = path.join(workspacePath, ".telagent", "project.json");
