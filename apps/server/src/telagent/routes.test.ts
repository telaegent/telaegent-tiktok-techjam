...
describe("Telagent routes", () => {
  it("returns 202 Operations while preserving existing Agent routes", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "telagent-route-test-"));
    temporaryDirectories.push(root);
    const store = new JsonStore(path.join(root, "launchpad.json"));
    await store.initialize();
    const orchestrator = new FakeOrchestrator();
    const telagent = new TelagentService(
      store,
      orchestrator,
      new FakeConflictEvaluator(),
    );
    const app = await createApp(loadConfig({ NODE_ENV: "test" }), agentService, telagent);

    const initialized = await app.inject({
      method: "POST",
      url: "/api/telagent/demo/initialize",
      payload: {},
    });
    ...
    const bobAccepted = await app.inject({
      method: "POST",
      url: "/api/telagent/conversations/conv_phoenix_demo/messages",
      payload: {
        ownerId: "bob",
        agentId: "bob-agent",
    ...
    expect(handle.pollUrl).toBe(
      `/api/telagent/operations/${handle.operationId}`,
    );
    ...
    const snapshot = await app.inject({
      method: "GET",
      url: "/api/telagent/projects/phoenix/snapshot",
    });
  ...
  it("validates IDs and returns a safe not-found response", async () => {
    ...
    const invalid = await app.inject({
      method: "GET",
      url: "/api/telagent/projects/not%2Fsafe/snapshot",
    });
    ...
    expect(missing.json()).toEqual({ error: "Telagent project not found" });
    await app.close();
  });
});
