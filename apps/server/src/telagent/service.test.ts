...
import { TelagentService } from "./service.js";
...
const createStore = async () => {
  const root = await mkdtemp(path.join(tmpdir(), "telagent-service-test-"));
...
describe("TelagentService", () => {
  it("initializes the demo idempotently and returns an ordered snapshot", async () => {
    const { store } = await createStore();
    const service = new TelagentService(store, new DeferredOrchestrator());
    ...
    expect(store.snapshot().telagent.projects).toHaveLength(1);
    expect(store.snapshot().telagent.conversations).toHaveLength(1);
    expect(store.snapshot().telagent.events).toHaveLength(1);
  });

  it("returns an Operation immediately and deduplicates retries", async () => {
    const { store } = await createStore();
    const orchestrator = new DeferredOrchestrator();
    const service = new TelagentService(
      store,
      orchestrator,
    ...
    expect(store.snapshot().telagent.operations).toHaveLength(1);
    expect(store.snapshot().telagent.conversationEntries).toHaveLength(1);
  ...
  it("rejects forbidden intent paths before publication", async () => {
    ...
    expect(store.snapshot().telagent.intents).toHaveLength(0);
  ...
  it("minimizes untyped snapshot records and rejects an unknown owner", async () => {
    ...
    await store.mutate((database) => {
      database.telagent.intents.push({
        intentId: "intent-1",
        projectId: "phoenix",
        task: "Safe task",
  ...
  it("stores a safe failure without persisting the raw runtime error", async () => {
    const { store, databasePath } = await createStore();
    const orchestrator = new DeferredOrchestrator();
    const service = new TelagentService(store, orchestrator);
    await service.initializeDemo();
    const accepted = await service.submitConversationMessage({
      ...submitInput,
  ...
  it("rejects secret-like shared content before it reaches persistence", async () => {
    ...
    expect(store.snapshot().telagent.operations).toHaveLength(0);
    expect(await readFile(databasePath, "utf8")).not.toContain(
      "super-secret-value",
    );
  });
  ...
  it("fails interrupted Operations on restart but preserves waiting requests", async () => {
    ...
    await store.mutate((database) => {
      database.telagent.operations.push(
