...
import {
  ...
  TELAGENT_ERROR_CODES,
  TELAGENT_LIMITS,
  TELAGENT_SCHEMA_VERSION,
  TELAGENT_TOOL_NAMES,
} from "./constants.js";
...
export const relativePathSchema = z
  .string()
  .min(1)
  .max(TELAGENT_LIMITS.pathLength)
...
const shortTextSchema = z.string().trim().min(1).max(TELAGENT_LIMITS.summaryLength);
const taskSchema = z.string().trim().min(1).max(TELAGENT_LIMITS.taskLength);
const purposeSchema = z.string().trim().min(1).max(TELAGENT_LIMITS.purposeLength);
const planStepsSchema = z
  .array(shortTextSchema)
  .max(TELAGENT_LIMITS.planSteps);
const filesSchema = z.array(relativePathSchema).max(TELAGENT_LIMITS.files);
const interfacesSchema = z
  .array(z.string().trim().min(1).max(200))
  .max(TELAGENT_LIMITS.interfaces);
const dependenciesSchema = z
  .array(z.string().trim().min(1).max(200))
  .max(TELAGENT_LIMITS.dependencies);
const blockersSchema = z.array(shortTextSchema).max(TELAGENT_LIMITS.blockers);
...
export const suggestResolutionInputSchema = z.strictObject({
  ...
  dependencyLinks: z.array(dependencyLinkSchema).max(TELAGENT_LIMITS.dependencies),
  requiredRules: z.array(shortTextSchema).min(1).max(12),
  rationale: z.string().trim().min(1).max(TELAGENT_LIMITS.rationaleLength),
});
export const requestContextInputSchema = z.strictObject({
  topic: shortTextSchema,
  purpose: purposeSchema,
  requestedPaths: z.array(pathRuleSchema).min(1).max(TELAGENT_LIMITS.approvedPathRules),
...
export const createContextPackInputSchema = z.strictObject({
  ...
  sources: z.array(contextPackSourceSchema).min(1).max(TELAGENT_LIMITS.sourceRefs),
...
const deliverySchema = z.strictObject({
  ...
  exchangeNumber: z
    ...
    .int()
    .min(1)
    .max(TELAGENT_LIMITS.maxExchangeNumber),
...
const evidenceSchema = z.strictObject({
  branch: branchSchema,
  baseCommit: commitSchema,
  sourceRefs: z.array(sourceRefSchema).max(TELAGENT_LIMITS.sourceRefs).optional(),
});
const envelopeCommon = {
  schemaVersion: z.literal(TELAGENT_SCHEMA_VERSION),
...
export const telagentEnvelopeSchema = z
...
export const telagentToolActionSchema = z.discriminatedUnion("name", [
...
const agentStep = (nextAction: z.ZodType) =>
  z.strictObject({
    publicSummary: z.string().trim().min(1).max(TELAGENT_LIMITS.summaryLength),
...
export const conversationEntrySchema = z.discriminatedUnion("type", [
  ...
  z.strictObject({ ...entryCommon, type: z.literal("tool_call"), payload: z.strictObject({ action: telagentToolActionSchema }) }),
  z.strictObject({ ...entryCommon, type: z.literal("tool_result"), payload: z.strictObject({ toolName: z.enum(TELAGENT_TOOL_NAMES), outcome: z.enum(["completed", "denied", "failed"]), safeSummary: shortTextSchema }) }),
  ...
  z.strictObject({ ...entryCommon, type: z.literal("error"), payload: z.strictObject({ code: z.enum(TELAGENT_ERROR_CODES), message: shortTextSchema, auditEventId: identifierSchema.optional() }) }),
]);
...
export const coordinationRequestSchema = z.strictObject({
  ...
  exchangeCount: z.number().int().min(0).max(TELAGENT_LIMITS.maxExchangeNumber),
...
export const contextDecisionSchema = z.strictObject({
  ...
  approvedPaths: z.array(pathRuleSchema).max(TELAGENT_LIMITS.approvedPathRules),
  decidedAt: utcTimestampSchema,
});
...
export const safeOperationErrorSchema = z.strictObject({
  code: z.enum(TELAGENT_ERROR_CODES),
...
export const operationSchema = z.strictObject({
  ...
  type: z.union([z.enum(TELAGENT_TOOL_NAMES), z.enum(["agent_run", "workflow"])]),
  projectId: projectIdSchema,
  conversationId: identifierSchema,
  requestId: identifierSchema,
  correlationId: identifierSchema,
...
export const idempotencyRecordSchema = z.strictObject({
  projectId: projectIdSchema,
  senderAgentId: identifierSchema,
  operation: z.enum(TELAGENT_TOOL_NAMES),
  idempotencyKey: identifierSchema,
...
export const telagentDatabaseSchema = z.strictObject({
...
export const operationAcknowledgementSchema = z.strictObject({
  ...
  correlationId: identifierSchema,
  state: z.enum(OPERATION_STATES),
  pollUrl: z.string().regex(/^\/api\/telagent\/operations\/[A-Za-z0-9._:-]+$/),
});
export const telagentErrorEnvelopeSchema = z.strictObject({
...
export const telagentHttpBodySchemas = {
