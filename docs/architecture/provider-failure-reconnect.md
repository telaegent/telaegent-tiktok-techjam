...
Provider text must never determine product state. The backend classifies the
failure, updates provider/session state, and returns server-selected actions.
...
- `not_connected`: CLI may be installed and authenticated, but no successful
  live probe exists for this runtime binding and provider.
- `connecting`: one deduplicated live probe is active.
- `connected`: the binding passed a live provider probe.
- `reconnect_required`: authentication is absent or expired.
- `unavailable`: CLI installation, provider availability, or a non-auth probe
  failed.
...
| --- | --- | --- | --- |
| `RUNTIME_AUTHENTICATION_FAILED` | Failed; provider reconnect required | Invalidate the binding's private provider session | `reconnect_provider`, `dismiss` |
...
continue private provider session
        ...
        -> delete exact user/repository/conversation/provider cache entry
...
There is no second recovery loop. Provider session IDs remain internal and
must not appear in progress events, logs, browser payloads, or public errors.

## Reconnect and revocation

Reconnect is explicit. After credentials are updated, the backend invalidates
the affected session cache and requires a successful live probe before marking
the binding connected again.
...
fabricate completion. Provider session references may be reused only if their
owner scope and runtime binding are still valid.
