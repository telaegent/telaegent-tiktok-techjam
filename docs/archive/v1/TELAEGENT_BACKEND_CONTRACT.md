# Telaegent Backend Orchestration Contract
...
Telaegent extends these existing files and responsibilities:
...
No Telaegent service or route may call an `AgentRunner` directly.
...
`TelaegentDatabase` contains these arrays:
...
1. Reading a version-1 database without `telaegent` normalizes it to the empty
...
- Mark Telaegent Operations associated with queued/running Runs failed or
...
- Preserve existing non-Telaegent routes unchanged.

## 10. Project snapshot contract

`GET /api/telaegent/projects/phoenix/snapshot` is the authoritative frontend
