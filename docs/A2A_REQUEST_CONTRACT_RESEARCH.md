# Defining an A2A request for Telaegent

**Decision memo for Duy's protocol workstream**  
**Research date:** 2026-08-28  
**Target:** Telaegent two-agent MVP on Fastify, TypeScript, Zod, and Vitest

## Executive decision

For Telaegent, an “A2A request” should be defined as two contracts joined by an adapter:

1. **Canonical Telaegent contract:** a strict, versioned `TelaegentEnvelopeV1` discriminated by `operation`, used by the permission, state, agreement, conflict, persistence, and queueing code.
2. **A2A interoperability contract:** the official A2A v1 `SendMessageRequest`, with the Telaegent envelope carried in a structured `Part.data` under a required, versioned Telaegent extension.
...
| Telaegent envelope | Duy | Stable domain meaning, identity claims, routing, expiry, evidence | Strict Zod schema |
...
```text
parse binding -> authenticate -> validate Telaegent data -> normalize identity
...
A2A does **not** define Telaegent's operation names, owner/recipient IDs, intent IDs, Git evidence, exchange limit, permission classes, approval versions, or ContextPack rules. Those belong in a Telaegent extension, not as modifications to the base A2A schema. A2A supports this through advertised, opt-in, versioned extension URIs; breaking extension changes require a new URI. [A2A extensions](https://a2a-protocol.org/latest/specification/#46-extensions)

## Recommended canonical Telaegent request
...
interface TelaegentEnvelopeV1<T extends TelaegentToolName> {
...
Implement the public union as one schema per operation, then combine it with `z.discriminatedUnion("operation", [...])`. Do not implement `payload` as `z.unknown()` or a generic record. Use `.strict()` on the Telaegent envelope and security-sensitive nested objects. Bound every identifier, string, array, and structured-data depth.
...
| `conversationId` | Telaegent/server, or accepted client context | Related coordination thread | `message.contextId` |
| `intentId` | Telaegent domain | Planned work identity | Telaegent `Part.data` only |
| `correlationId` | Ingress/tracing layer | Logs and audit correlation; no replay semantics | A2A request metadata or trace context |
| `idempotencyKey` | Caller, validated by server | Deduplicates one logical mutation | Telaegent extension; not A2A core |
| `operationId` | Telaegent server | Durable async execution record | Usually maps to an A2A `taskId` at the adapter |
...
A2A-Version: 1.0
A2A-Extensions: urn:telaegent:extension:coordination:v1
    ...
    "parts": [
      {
        "data": {
          "schemaVersion": "telaegent.v1",
          "requestId": "req_01JTELAEGENT42",
          "correlationId": "corr_01JTELAEGENT42",
          "idempotencyKey": "idem_01JTELAEGENT42",
          "projectId": "phoenix",
        ...
        "mediaType": "application/vnd.telaegent.request+json"
      }
    ],
    "extensions": ["urn:telaegent:extension:coordination:v1"]
  },
  "configuration": {
    "acceptedOutputModes": ["application/vnd.telaegent.result+json"],
    "historyLength": 0,
...
- Require the extension in both the negotiated header and `message.extensions` if Telaegent semantics are mandatory.
- Require exactly one Telaegent request `data` Part for this extension version.
...
### Telaegent MVP endpoint

```http
HTTP/1.1 202 Accepted
Location: /api/operations/op_01JTELAEGENT42
Retry-After: 2
...
```json
{
  "operationId": "op_01JTELAEGENT42",
  "requestId": "req_01JTELAEGENT42",
  "correlationId": "corr_01JTELAEGENT42",
  "status": "accepted",
  "statusUrl": "/api/operations/op_01JTELAEGENT42",
  "expiresAt": "2026-08-28T04:15:00.000Z"
}
```

RFC 9110 says a 202 response is noncommittal and ought to describe current status and point to a status monitor, but HTTP does not define Telaegent's polling lifecycle. Make `statusUrl` the internal contract. Document `Location` and `Retry-After` as Telaegent conventions, including whether the interval is a minimum poll delay. [RFC 9110, 202 Accepted](https://www.rfc-editor.org/rfc/rfc9110.html#name-202-accepted)

### A2A adapter

An A2A `SendMessage` response contains exactly one `Task` or one direct `Message`. Because Telaegent work is asynchronous and stateful, normally return an in-progress Task when `returnImmediately` is true, and expose later updates through Get Task polling first. Add SSE or push only after the relevant Agent Card capability is implemented. [A2A SendMessage response and configuration](https://raw.githubusercontent.com/a2aproject/A2A/v1.0.1/specification/a2a.proto#L136-L177)

Map Telaegent states deliberately:

| Telaegent Operation status | A2A Task state | Note |
|---|---|---|
...
| `failed`, `expired` | `TASK_STATE_FAILED` | Preserve Telaegent error code in typed extension data |
...
Freeze stronger Telaegent behavior:
...
These details are Telaegent policy. They are informed by the IETF Idempotency-Key draft, but that draft expired on 2026-04-18 and must not be presented as an Internet Standard. [Expired Idempotency-Key draft-07](https://datatracker.ietf.org/doc/html/draft-ietf-httpapi-idempotency-key-header-07)

## Validation and security rules

At minimum, the Zod layer and route/service boundary should enforce:

- exact `schemaVersion: "telaegent.v1"` and, at the adapter, `A2A-Version: 1.0`;
...
Keep `duy.md`'s safe Telaegent envelope for internal routes. It should include `code`, safe `message`, `correlationId`, and optional `auditEventId`, with no raw stack or provider stderr.
...
| Telaegent error | A2A/HTTP behavior |
|---|---|
...
| `INVALID_STATE`, `STALE_VERSION`, idempotency reuse | HTTP 400 or binding-appropriate typed application error; preserve Telaegent subcode in extension details |
...
- valid request parses and round-trips through the Telaegent domain type;
- all 11 `operation` values accept only their matching payload;
- A2A adapter accepts one correctly typed `data` Part;
- duplicate/missing Telaegent Part and unsupported extension version fail;
...
- A2A `AUTH_REQUIRED` cannot activate a Telaegent approval;
...
- accepted Telaegent request returns 202 with a usable `statusUrl`;
...
- every Telaegent state maps to the intended A2A state;
...
2. Define `PayloadByOperation`, the 11 strict payload schemas, and the `TelaegentEnvelopeV1` discriminated union.
...
- the stable Telaegent extension URI and where its schema will be published;
...
- which Telaegent error details are safe to expose through an external A2A adapter;
...
The most useful definition is not “Telaegent equals A2A.” It is: **Telaegent owns a strict domain request; A2A owns an optional interoperable transport envelope.** Freeze the internal schema and trust rules now, then add a thin A2A v1 adapter without allowing A2A metadata, model output, or caller claims to bypass Telaegent's deterministic permission and state engines.
