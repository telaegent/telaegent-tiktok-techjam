# Telagent protocol contract v1

Status: frozen Day-0 contract for the two-agent Phoenix MVP  
Owner: Duy  
Logical schema version: `telagent.v1`

## Contract boundary

All Agent-originated operations pass through the strict `TelagentRequest` union in `apps/server/src/telagent/types.ts` and `telagentEnvelopeSchema` in `schemas.ts`.
...
| `conversationId` | Telagent | Related coordination entries |
| `intentId` | Telagent | One owner/Agent work intent |
| `operationId` | Telagent server | Durable asynchronous execution |
| `artifactId` | Telagent server | Validated ContextPack/result artifact |

IDs are bounded opaque strings. They are not credentials and never replace authorization.

## Envelope invariants

- Exact version `telagent.v1`.
...
The eight committed JSON Schemas in `apps/server/src/telagent/output-schemas/` are generated from the authoritative Zod schemas and tested for synchronization.
...
- Khoa: `types.ts`, `constants.ts`, `state-machine.ts`, `errors.ts`, and `EMPTY_TELAGENT_DATABASE`.
