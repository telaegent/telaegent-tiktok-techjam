...
import { TELAEGENT_ENVELOPE_FIXTURES } from "./contract-fixtures.js";
import {
...
import { telaegentEnvelopeSchema } from "./schemas.js";
import type { TelaegentRequest } from "./types.js";

const publish = (): TelaegentRequest =>
  structuredClone(TELAEGENT_ENVELOPE_FIXTURES.relay_publish_intent);
...
describe("reply inheritance", () => {
  const original = () =>
    telaegentEnvelopeSchema.parse({
      ...TELAEGENT_ENVELOPE_FIXTURES.relay_ask_status,
      requestId: "req_status_01",
      delivery: {
        ...TELAEGENT_ENVELOPE_FIXTURES.relay_ask_status.delivery,
        exchangeNumber: 1,
      },
    });
  const reply = () =>
    telaegentEnvelopeSchema.parse({
      ...TELAEGENT_ENVELOPE_FIXTURES.relay_reply,
      sender: { ownerId: "bob", agentId: "bob-agent", provider: "codex" },
      recipient: { ownerId: "alice", agentId: "alice-agent" },
      delivery: {
        ...TELAEGENT_ENVELOPE_FIXTURES.relay_reply.delivery,
        exchangeNumber: 2,
