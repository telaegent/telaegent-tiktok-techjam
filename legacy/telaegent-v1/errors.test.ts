...
import { telaegentErrorEnvelopeSchema } from "./schemas.js";

describe("Telaegent protocol errors", () => {
  it("freezes every code to its required HTTP status", () => {
    expect(TELAEGENT_ERROR_HTTP_STATUS).toEqual({
      INVALID_REQUEST: 400,
      POLICY_DENIED: 403,
      NOT_FOUND: 404,
      INVALID_STATE: 409,
  ...
  it("serializes only safe error fields", () => {
    ...
    expect(telaegentErrorEnvelopeSchema.parse(envelope)).toEqual(envelope);
