-- Surface the GitHub avatar captured during Telaegent OAuth in project-scoped
-- collaborator discovery. The account identity is optional here so existing
-- connector-only test data remains discoverable; real browser accounts receive
-- their current GitHub avatar whenever they sign in.
  ...
  select case
    when p_limit is null or p_limit not between 1 and 51 or
      ...
      from (
        select
          peer.user_id,
          connection_identity.github_login,
          account_identity.avatar_url,
        ...
        join public.github_repository_access access
         ...
         and access.github_repository_id = project.github_repository_id
         and access.status = 'verified'
        join public.github_connections connection_identity
          on connection_identity.user_id = access.user_id
         and connection_identity.github_connection_id = access.github_connection_id
         and connection_identity.status = 'connected'
        left join public.account_github_identities account_identity
          on account_identity.user_id = peer.user_id
        ...
        order by connection_identity.github_login
        limit p_limit
      ) listed
    ), '[]'::jsonb)
  end;
