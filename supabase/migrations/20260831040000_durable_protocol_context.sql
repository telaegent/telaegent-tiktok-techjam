-- Reconstructs one private sender turn solely from durable Telaegent rows.
-- Provider sessions remain a local cache and are never queried or persisted.
...
revoke all on function public.load_sender_protocol_context(
  uuid, bigint, uuid, uuid, integer
) from public, anon, authenticated;
grant execute on function public.load_sender_protocol_context(
  uuid, bigint, uuid, uuid, integer
) to service_role;
